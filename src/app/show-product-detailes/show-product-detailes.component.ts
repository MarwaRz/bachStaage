...
export class ShowProductDetailesComponent implements OnInit {
 ...
 this.showTable
  ...
  deleteProduct(idFormation){
    this.productService.deleteProduct(idFormation).subscribe(
      (resp)=> {
        this.getAllProducts();
      },
      (error: HttpErrorResponse) => {
  ...
  deleteFormateur(idFormation) {
    this.productService.deleteProduct(idFormation)
      .subscribe(
        data => {
          console.log(data);
          this.getAllProducts();
  ...
  addcycle(idFormation):void{
    this.router.navigate(['imprimer', idFormation])  }

  editProductDetails(idFormation){
    this.router.navigate(['/addNewProduct',{id: idFormation}])
  }
